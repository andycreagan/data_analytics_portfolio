...
-- Data transformation - Grouping by metropolitan area or region: After working in Tableau, I noticed that several regions were categorized incorrectly. I used the same UPDATE, SET, and WHERE statements used above to update the 'Total US', 'West', 'West Texas - New Mexico', and 'South Carolina' regions to have the 'Region' value assigned in the metropolitan_region column. Once these changes were made in avocado_prices_stage and avocado_prices_final, I 
...
SET
  metropolitan_region = 'Region'
WHERE
  region IN ('Total US', 'West', 'West Texas - New Mexico', 'South Carolina');
...
-- Re-creating final data extracts for analysis in Tableau: Since there were several miscategorized regions in my original metropolitan_area_final and region_final tables, I created two new tables named metropolitan_area_final_2 and region_final_2 that contained the updated and correct data. I then downloaded the csv files from both tables so that I could use the updated data in Tableau.

CREATE TABLE 
  avocado-prices-435314.avocado_prices.metropolitan_area_final_2 AS (
    SELECT
      *
    FROM
      avocado-prices-435314.avocado_prices.avocado_prices_final
...
CREATE TABLE 
  avocado-prices-435314.avocado_prices.region_final_2 AS (
    SELECT
      *
    FROM
      avocado-prices-435314.avocado_prices.avocado_prices_final
...
SELECT
  *
FROM
  avocado-prices-435314.avocado_prices.metropolitan_area_final_2;

SELECT
  *
FROM
  avocado-prices-435314.avocado_prices.region_final_2;